...
class MidiConnector:
    ...
    def load_file(filename):
        ...
        if not 0 < midi_data.ticks_per_beat < 90028: #was 128, I don't think this is relevant anyway
        ...
        for message in tempo_map:
            ...
            if not isinstance(message, mido.MetaMessage):
                #raise MidiInvalidException('Tempo map should not contains notes') #mais pète un coup
                pass
            if message.type in MidiConnector.META_INFO_TYPES:
                ...
                pass  # TODO
            else: #something else is not always an error, might just be info
        ...
        for i, track in enumerate(midi_data.tracks[1:]):  # We ignore the tempo map
            ...
            for message in track:
                ...
                if isinstance(message, mido.MetaMessage):  # Lyrics, track name and other meta info
                    if message.type in MidiConnector.META_INFO_TYPES:
                        ...
                        #raise MidiInvalidException('Track {} should not contain {}'.format(i, message.type)) #elle fait bien ce qu'elle veut la track
                        pass
                    else:
                        err_msg = 'Track {} contains unsupported meta-message type ({})'.format(i, message.type)
                        #raise MidiInvalidException(err_msg)
                    # What about 'sequence_number', cue_marker ??? franchement ta gueule
                else:  # Note event
                    if message.type == 'note_on' and message.velocity != 0:  # Note added
                        ...
                        # Ignored
                        pass
                    elif message.type == 'sysex': #System Exclusive, ignore that
                        pass
                    else:
            ...
            if buffer_notes:  # All notes should have ended
                ...
                #could implement something to make sure all notes end, but is it necessary ?
                pass
            ...
            if new_track.is_drum:
                ...
                pass # was continue, but we may not want to ignore all tracks
